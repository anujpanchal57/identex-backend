...
class Product:
    ...
    def get_lot_products(self, lot_id):
        try:
            self.__cursor.execute("""select p.reqn_product_id, pm.product_id, pm.product_name, pm.product_category, 
                                    p.product_description, p.quantity, p.unit, pm.product_sub_category
                                    from products as p 
